...
export async function logAudit({ actor, action, targetType = null, targetId = null, riskLevel = 'low', before = null, after = null, requestId = null }) {
...
export async function queryAuditLogs(searchParams) {
  ...
  return {
    rows: result.rows.map((row) => ({ ...row, before: row.before_json ? JSON.parse(row.before_json) : null, after: row.after_json ? JSON.parse(row.after_json) : null })),
    meta: result.meta,
  };
}
