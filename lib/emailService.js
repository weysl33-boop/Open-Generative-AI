...
export async function sendEmailSmtpTest({ to }) {
  ...
  try {
    ...
    const result = await transport.sendMail({
      from: { name: created.config.fromName || 'KoyoSIM', address: created.config.fromEmail },
      to: recipient,
    ...
    if (!Array.isArray(result.accepted) || result.accepted.length === 0) {
      throw Object.assign(new Error('SMTP server did not accept the test recipient'), { code: 'SMTP_REJECTED' });
    }
    return { success: true, latencyMs: Date.now() - startedAt };
  } catch (error) {
    const code = error?.code === 'EMAIL_INVALID_RECIPIENT'
      ? error.code
      : error?.code === 'SMTP_NOT_CONFIGURED' ? error.code : smtpErrorCode(error);
    console.error('[email/smtp-test]', { code });
    throw Object.assign(new Error('测试邮件发送失败，请检查后台 SMTP 配置和收件地址'), { code });
  } finally {
    transport?.close();
  }
}
...
export async function sendEmailVerificationCode({ to, code }) {
  ...
  let transport;
  try {
    const created = await createEmailTransport({ requireEnabled: true });
    transport = created.transporter;
    const result = await transport.sendMail({
      from: { name: created.config.fromName || 'KoyoSIM', address: created.config.fromEmail },
      to: recipient,
    ...
    if (!Array.isArray(result.accepted) || result.accepted.length === 0) {
      throw Object.assign(new Error('SMTP server did not accept the verification recipient'), { code: 'SMTP_REJECTED' });
    }
    return { success: true };
  } catch (error) {
    const code = error?.code === 'SMTP_NOT_CONFIGURED' ? error.code : smtpErrorCode(error);
    console.error('[email/verification-send]', { code });
    throw Object.assign(new Error('验证码邮件发送失败，请稍后重试'), { code });
  } finally {
    transport?.close();
  }
}
