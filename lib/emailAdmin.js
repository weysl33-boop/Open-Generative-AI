...
export async function saveEmailSmtpPassword({ actor, adminPassword, password, requestId }) {
  if (!(await verifyAdminPassword(actor.id, adminPassword))) return { error: '管理员密码验证失败' };
  const cleanPassword = typeof password === 'string' ? password.trim() : '';
  if (cleanPassword.length < 8 || cleanPassword.length > 1024) return { error: 'SMTP 客户端密码长度无效' };
  await providerRepo.saveProviderSecret({ provider: EMAIL_SMTP_PROVIDER, name: 'smtp_password', secretValue: cleanPassword });
  await logAudit({
    actor,
...
export async function sendEmailSmtpTest({ actor, to, requestId }) {
  ...
  const result = await deliverEmailSmtpTest({ to: recipient });
