...
export async function listCreations(searchParams) {
  ...
  if (q) {
    clauses.push(`(u.id ILIKE ${nextParameter()} OR u.email ILIKE ${nextParameter()} OR c.user_id ILIKE ${nextParameter()} OR c.id ILIKE ${nextParameter()} OR c.label ILIKE ${nextParameter()} OR c.model ILIKE ${nextParameter()})`);
...
export async function listFailedCreations(searchParams) {
  ...
  const q = String(params.get('q') || '').trim();
  if (q) {
    clauses.push(`(u.id ILIKE ${nextParameter()} OR u.email ILIKE ${nextParameter()} OR c.error_code ILIKE ${nextParameter()} OR c.error_reason ILIKE ${nextParameter()} OR c.model ILIKE ${nextParameter()})`);
...
export async function listQueuedGenerationIds(batchSize = 10) {
  // 加速卡的真实效果就在这一行 ORDER BY：有效期内用户的任务先出队，其余仍按提交顺序 FIFO。
  return queryMany(`
    SELECT id
    FROM creations
    WHERE status = 'queued'
    ORDER BY EXISTS (
      SELECT 1 FROM auth_usr.users u
      WHERE u.id = creations.user_id AND u.priority_until > now()
