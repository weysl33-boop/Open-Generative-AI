...
export async function toggleCommunityLike(postId, userId) {
  return await withTransaction(async (client) => {
    ...
    const postRow = await client.query('SELECT likes_count FROM ai_studio.community_posts WHERE id = $1', [postId]);
    const likesCount = postRow.rows[0]?.likes_count || 0;
...
export async function addCommunityComment({ postId, userId, content }) {
  ...
  return await withTransaction(async (client) => {
    ...
    const userRow = await client.query(
      'SELECT display_name, avatar_url, username FROM auth_usr.users WHERE id = $1',
      [userId]
    );
