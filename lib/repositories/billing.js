...
export async function listSubscriptions(searchParams) {
  ...
  if (q) {
    clauses.push(`(u.user_number ILIKE ${nextParam()} OR u.email ILIKE ${nextParam()} OR s.user_id ILIKE ${nextParam()} OR s.id ILIKE ${nextParam()})`);
...
export async function listOrders(searchParams) {
  ...
  const q = String(params.get('q') || '').trim();
  if (q) {
    clauses.push(`(u.user_number ILIKE ${nextOrderParam()} OR u.email ILIKE ${nextOrderParam()} OR o.user_id ILIKE ${nextOrderParam()} OR o.id ILIKE ${nextOrderParam()} OR o.provider_order_id ILIKE ${nextOrderParam()})`);
