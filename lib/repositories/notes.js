...
export async function addNote({ targetType, targetId, body, authorId, authorEmail, transaction = null }) {
  const id = randomId('note');
  const cleanBody = body.trim();
  const now = nowIso();
  const run = transaction?.execute ? transaction.execute.bind(transaction) : execute;
  await run(`INSERT INTO admin_notes (id, target_type, target_id, body, author_id, author_email, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, targetType, String(targetId), cleanBody, authorId, authorEmail, now]);
