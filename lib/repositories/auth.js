...
export async function updateUserOnboarding({ userId, displayName = null, zodiac = null, industry = null, occupation = null, preferences = null }, transaction = null) {
  return one(transaction)(`
    UPDATE users SET
      display_name = COALESCE($1, display_name),
      zodiac = COALESCE($2, zodiac),
      industry = COALESCE($3, industry),
      occupation = COALESCE($4, occupation),
      preferences_json = COALESCE($5::jsonb, preferences_json),
      onboarding_completed = TRUE,
      onboarded_at = COALESCE(onboarded_at, NOW()),
      updated_at = NOW()
    WHERE id = $6
    RETURNING id, display_name, zodiac, industry, occupation, preferences_json, onboarding_completed, onboarded_at
  `, [displayName, zodiac, industry, occupation, preferences ? JSON.stringify(preferences) : null, userId]);
}
