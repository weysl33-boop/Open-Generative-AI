...
export async function findUserForUpdate(userId, transaction = null) {
  return one(transaction)(`SELECT id, uuid, email, phone, phone_country_code, phone_verified_at, display_name, avatar_url, role, credits, status
...
export async function updateUserPhoneIdentity({ userId, phone, countryCode, verifiedAt }, transaction = null) {
  return one(transaction)(`UPDATE users
    ...
    WHERE id = $4
    RETURNING id, uuid, email, phone, phone_country_code, phone_verified_at, display_name, avatar_url, role, credits, status`,
...
export async function findUserSession(tokenHash) {
  return queryOne(`SELECT u.id, u.uuid, u.user_number, u.email, u.phone, u.phone_country_code, u.display_name, u.avatar_url, u.role, u.credits, u.status, u.is_activity_public, u.privacy_settings, u.locale,
