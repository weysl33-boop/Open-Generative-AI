...
export async function listCreditLedger(searchParams) {
  ...
  if (q) {
    clauses.push(`(u.user_number ILIKE ${nextParameter()} OR u.email ILIKE ${nextParameter()} OR l.user_id ILIKE ${nextParameter()} OR l.description ILIKE ${nextParameter()} OR l.reference_id ILIKE ${nextParameter()})`);
  ...
  const baseSql = `
    SELECT l.id, l.user_id, l.delta, l.description AS reason, l.reference_id, l.actor_user_id,
           l.bucket_type, l.action_type, l.balance_after, l.created_at, u.user_number, u.email, u.avatar_url
