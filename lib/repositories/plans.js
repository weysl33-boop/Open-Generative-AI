...
export function parseFeaturesAndMeta(value, planId = '') {
  ...
  // 套餐基准默认值 fallback (参考图4对齐月付、季付、年付)
  const defaultsByPlan = {
    starter: { monthlyCny: 140, quarterlyCny: 350, quarterlyUsd: 50, yearlyCny: 999, yearlyUsd: 149, quotaBase: 2000, quotaBonus: 400, concurrency: 5, asyncConcurrency: 10, portraitCapacity: 2, badge: '多送20%' },
    basic: { monthlyCny: 350, quarterlyCny: 890, quarterlyUsd: 125, yearlyCny: 2868, yearlyUsd: 399, quotaBase: 5000, quotaBonus: 1500, concurrency: 7, asyncConcurrency: 20, portraitCapacity: 5, badge: '多送30%' },
    plus: { monthlyCny: 700, quarterlyCny: 1780, quarterlyUsd: 250, yearlyCny: 6108, yearlyUsd: 849, quotaBase: 10000, quotaBonus: 4000, concurrency: 10, asyncConcurrency: 50, portraitCapacity: 10, badge: '多送40% · 人气推荐' },
    pro: { monthlyCny: 1400, quarterlyCny: 3580, quarterlyUsd: 500, yearlyCny: 11988, yearlyUsd: 1699, quotaBase: 20000, quotaBonus: 10000, concurrency: 15, asyncConcurrency: 80, portraitCapacity: 20, badge: '多送50% · 旗舰大师' },
    free: { monthlyCny: 0, quarterlyCny: 0, quarterlyUsd: 0, yearlyCny: 0, yearlyUsd: 0, quotaBase: 0, quotaBonus: 0, concurrency: 2, asyncConcurrency: 3, portraitCapacity: 0, badge: '免费体验' },
  };

  const idKey = String(planId || '').toLowerCase();
  const def = defaultsByPlan[idKey] || { monthlyCny: 0, quarterlyCny: 0, quarterlyUsd: 0, yearlyCny: 0, yearlyUsd: 0, quotaBase: 0, quotaBonus: 0, concurrency: 2, asyncConcurrency: 3, portraitCapacity: 0, badge: '' };

  return {
    features,
    meta: {
      quarterlyCny: Number(meta.quarterlyCny ?? meta.quarterly_cny ?? def.quarterlyCny),
      quarterlyUsd: Number(meta.quarterlyUsd ?? meta.quarterly_usd ?? def.quarterlyUsd),
...
export function getPlanPrice(plan, billingCycle = 'monthly', currency = 'CNY') {
  if (!plan) return 0;
  const isRmb = String(currency || 'CNY').toUpperCase() === 'CNY';
  const cycle = String(billingCycle || 'monthly').toLowerCase();
  if (cycle === 'yearly') {
    return isRmb ? Number(plan.yearlyCny || plan.meta?.yearlyCny || (plan.monthlyCny ? plan.monthlyCny * 12 * 0.7 : 0)) : Number(plan.yearlyUsd || plan.meta?.yearlyUsd || (plan.monthlyUsd ? plan.monthlyUsd * 12 * 0.7 : 0));
  }
  if (cycle === 'quarterly') {
    return isRmb ? Number(plan.quarterlyCny || plan.meta?.quarterlyCny || (plan.monthlyCny ? plan.monthlyCny * 3 * 0.85 : 0)) : Number(plan.quarterlyUsd || plan.meta?.quarterlyUsd || (plan.monthlyUsd ? plan.monthlyUsd * 3 * 0.85 : 0));
  }
  return isRmb ? Number(plan.monthlyCny || 0) : Number(plan.monthlyUsd || 0);
}
...
export function mapPlanConfig(row) {
  ...
  return {
    ...
    quarterlyCny: meta.quarterlyCny,
    quarterlyUsd: meta.quarterlyUsd,
