...
export async function listActiveSessions(searchParams) {
  ...
  const baseSql = `
    SELECT s.token_hash, s.user_id, s.expires_at, s.last_seen_at, s.created_at, u.email, u.role
    FROM sessions s JOIN users u ON u.id = s.user_id
    WHERE ${clauses.join(' AND ')}
  `;
  return pagedQuery({ baseSql, params: sqlParams, searchParams: params, tableAlias: 's' });
}
