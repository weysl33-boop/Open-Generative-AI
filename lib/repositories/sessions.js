...
export async function listActiveSessions(searchParams) {
  ...
  if (q) {
    clauses.push(`(u.user_number ILIKE ${next()} OR u.email ILIKE ${next()} OR s.user_id ILIKE ${next()})`);
    sqlParams.push(`%${q}%`, `%${q}%`, `%${q}%`);
  }
  const baseSql = `
    SELECT s.token_hash, s.token_hash AS id, s.user_id, s.expires_at, s.last_seen_at, s.created_at,
           u.user_number, u.email, u.avatar_url, u.role
