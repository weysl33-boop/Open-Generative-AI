...
export async function findUserById(id, transaction = null) {
  ...
  const user = await one(`
    SELECT id, uuid, user_number, email, phone, phone_country_code, display_name, avatar_url, role, credits, status,
...
export async function findUserByIdOrNumber(identifier) {
  ...
  if (/^\d{6}$/.test(clean)) {
    return await queryOne(`
      SELECT id, uuid, user_number, email, phone, display_name, avatar_url, role, credits, status,
