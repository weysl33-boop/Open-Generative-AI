...
export async function findUserById(id, transaction = null) {
  ...
  const user = await one(`
    SELECT id, email, phone, phone_country_code, display_name, avatar_url, avatar_frame, role, credits, status,
...
export async function findUserByPublicId(identifier) {
  const clean = String(identifier || '').trim();
  if (!/^[1-9]\d{5,63}$/.test(clean)) return null;
  return await queryOne(`
    SELECT id, email, phone, display_name, avatar_url, avatar_frame, role, credits, status,
...
export async function findUserByEmail(email) {
  return await queryOne(`
    SELECT id, email, phone, display_name, role, credits, status, last_login_at, updated_at, created_at
...
export async function getUserProfile(userId) {
  const profile = await queryOne(`
    SELECT id, username, email, phone, phone_country_code, locale, display_name, avatar_url,
...
export async function updateUserPrivacySettings(userId, { isActivityPublic, privacySettings }) {
  ...
  return await queryOne(`
    ...
    SET ${updates.join(', ')}
    WHERE id = $1
    RETURNING id, is_activity_public, privacy_settings
  `, values);
}
...
export async function listUsers(searchParams) {
  ...
  if (q) {
    clauses.push(`(id ILIKE ${nextParam()} OR email ILIKE ${nextParam()} OR display_name ILIKE ${nextParam()} OR phone ILIKE ${nextParam()})`);
...
export async function listAdmins() {
  const res = await query(`
    SELECT id, email, display_name, avatar_url, role, status, last_login_at, created_at
    FROM users
    WHERE role != 'user'
    ORDER BY created_at ASC
  `);
