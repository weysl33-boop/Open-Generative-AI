...
import { allocateUserId } from '../auth/user-id.js';
...
async function generateUserId(tx) {
  return allocateUserId({
    reserve: async (userId, digitLength) => Boolean(await authRepo.reserveUserId(userId, digitLength, tx)),
    findFirstAvailable: (digitLength) => authRepo.findFirstAvailableUserId(digitLength, tx),
    isExhausted: (digitLength) => authRepo.isUserIdLengthExhausted(digitLength, tx),
  });
}

async function insertUserWithAllocatedId(user, tx) {
  const id = await generateUserId(tx);
  const displayName = typeof user.displayName === 'function' ? user.displayName(id) : user.displayName;
  return authRepo.insertUser({ ...user, id, displayName }, tx);
}

function defaultDisplayName({ email, phone, provider, userId }) {
  if (userId) return `创作者#${userId}`;
...
export async function findOrCreateUserByPhone({
  ...
  return withTransaction(async (tx) => {
    ...
    const user = await insertUserWithAllocatedId({
      phone: fullPhone,
      phoneCountryCode: normalized.callingCode,
      displayName: (userId) => `用户#${userId}`,
      registrationSource,
      lastLoginAt: now,
      lastLoginIp: ip,
      createdAt: now,
...
export async function createUser(email, password, { registrationSource = 'web', ip = null } = {}) {
  ...
  const result = await withTransaction(async (tx) => {
    ...
    const user = await insertUserWithAllocatedId({
      ...
      passwordHash: hash,
      passwordSalt: salt,
      displayName: (userId) => defaultDisplayName({ email: normalized, userId }),
      registrationSource,
      lastLoginAt: now,
      lastLoginIp: ip,
      createdAt: now,
  ...
  if (result?.user?.email) {
    // 异步触发邮箱注册欢迎邮件（fire-and-forget，不阻塞用户注册响应）
    queueMicrotask(() => {
      import('../emailMarketing.js').then(({ sendWelcomeEmail }) => {
        return sendWelcomeEmail({
          to: result.user.email,
          name: result.user.displayName || result.user.display_name,
          userId: result.user.id,
        });
      }).catch((err) => {
        console.error('[auth/welcome-email-hook-failed]', { userId: result.user.id, error: err?.message });
      });
    });
  }
...
export async function createOAuthUser({ provider, providerUserId, email, emailVerified = false, displayName, avatarUrl, profile = null, ip = null }) {
  ...
  return withTransaction(async (tx) => {
    ...
    const user = await insertUserWithAllocatedId({
      email: trustedEmail,
      displayName: displayName || ((userId) => defaultDisplayName({ email: trustedEmail, provider, userId })),
...
export async function getUserBySession(token) {
  ...
  return {
    id: row.id,
    displayName: row.display_name || (row.phone ? `用户${row.phone.slice(-4)}` : row.email?.split('@')[0]) || `创作者#${row.id}`,
    ...
    avatarFrames: row.avatar_frames || [],
