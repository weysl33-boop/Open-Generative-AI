...
import { findPublicPlanById, getPlanPrice } from '../repositories/plans.js';
...
import { logPaymentEvent } from '../payments/paymentLog.js';
import { findCreditPackById } from '../payments/creditPacks.js';

/** 二维码有效期：支付宝与 timeout_express 同源，微信沿用页面既有节奏。 */
const QR_TTL_MS = { alipay: 30 * 60 * 1000, wechat: 15 * 60 * 1000 };
...
export async function createPaymentCheckout({ request, user, planId, productId, productType = 'subscription', provider = 'stripe', idempotencyKey, billingCycle = 'monthly' }) {
  ...
  // 人民币金额以分为单位，美元以美分为单位；根据订阅周期和币种精确计算应付金额
  const planPrice = normalizedProductType === 'credit_pack'
    ? (isRmb ? plan.priceCny : (plan.priceUsd || plan.priceCny / 7))
    : getPlanPrice(plan, normalizedCycle, currency);
  const amountMinor = Math.round((Number(planPrice) || (isRmb ? 29 : 9.9)) * 100);
  ...
  logPaymentEvent('payment.order.created', {
    orderId: order.id,
    outTradeNo: order.id,
  ...
  if (normalizedProvider === 'wechat') {
    try {
      ...
      return {
        ...
        creditAmount,
        status: 'PENDING',
        expiresAt: session.expiresAt || new Date(Date.now() + QR_TTL_MS.wechat).toISOString(),
  ...
  if (normalizedProvider === 'alipay') {
    try {
      ...
      return {
        ...
        creditAmount,
        status: 'PENDING',
        expiresAt: session.expiresAt || new Date(Date.now() + QR_TTL_MS.alipay).toISOString(),
        idempotent: created.idempotent,
      };
    } catch (error) {
