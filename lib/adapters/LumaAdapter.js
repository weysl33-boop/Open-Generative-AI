...
export class LumaAdapter extends BaseAdapter {
  ...
  async healthCheck() {
    if (!this.apiKey) {
      return { status: 'unhealthy', latencyMs: 0, message: 'Luma API Key 缺失' };
    }
    ...
    try {
      ...
      if (response.ok || response.status === 200) {
        return { status: 'healthy', latencyMs, message: 'Luma Dream Machine 官方 API 正常在线' };
      }
      return { status: 'degraded', latencyMs, message: `Luma 返回 HTTP ${response.status}` };
    } catch (err) {
