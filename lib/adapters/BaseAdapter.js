...
export class BaseAdapter {
  ...
  /**
   * 诊断健康检查。
   * probeKind 必须是 'credential'（只校验本机凭据）或 'http'（真的请求过上游）：
   * 后台要区分「上游确认在线」和「我们只是有密钥」，把前者说成后者就是假数据。
   */
  async healthCheck() {
    const startedAt = Date.now();
    try {
      if (!this.apiKey) {
        return { healthy: false, latencyMs: 0, probeKind: 'credential', message: '未配置 API Key 凭据' };
      }
