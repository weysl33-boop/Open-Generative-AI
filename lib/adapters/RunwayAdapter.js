...
export class RunwayAdapter extends BaseAdapter {
  ...
  async healthCheck() {
    if (!this.apiKey) {
      return { status: 'unhealthy', latencyMs: 0, message: 'Runway API Key 缺失' };
    }
    ...
    try {
      ...
      if (response.ok || response.status === 200) {
        return { status: 'healthy', latencyMs, message: 'Runway 官方 API 正常在线' };
      }
      return { status: 'degraded', latencyMs, message: `Runway 返回 HTTP ${response.status}` };
    } catch (err) {
