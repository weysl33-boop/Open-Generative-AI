...
// 进程内 OAuth State 临时存储（10分钟），用于应对跨站重定向丢 Cookie、浏览器隐私保护限制、无 www/有 www 切换等场景
const oauthStateCache = new Map();

function cleanupExpiredStates() {
  const now = Date.now();
  for (const [key, item] of oauthStateCache.entries()) {
...
if (typeof setInterval !== 'undefined') {
  const timer = setInterval(cleanupExpiredStates, 60000);
  timer.unref?.();
}
...
export function readOAuthCallbackState(cookieValue, returnedState, provider) {
  ...
  if (state && state.provider === provider && state.nonce && state.nonce === returnedState) {
    oauthStateCache.delete(returnedState);
    return state;
  }

  // 2. 兜底方案：如果 Cookie 在跨站重定向中丢失，从服务端缓存中通过 returnedState (nonce) 还原
  if (returnedState) {
    const cached = oauthStateCache.get(returnedState);
    if (cached) {
      oauthStateCache.delete(returnedState);
      if (cached.expiresAt >= Date.now() && cached.state?.provider === provider) {
        return cached.state;
      }
    }
  }
