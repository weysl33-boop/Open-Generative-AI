...
export async function dailyCheckIn(userId) {
  ...
  return await withTransaction(async (tx) => {
    ...
    if (existing) {
      // 没有 code 的 Error 会被 lib/security/publicError.js 的白名单吞成「失败」，
      // 用户看到的就是「签到没领到积分」；这条状态是可预期业务结果，必须带码出站。
      throw Object.assign(new Error('今日已完成签到，明天再来领取'), { code: 'ALREADY_CHECKED_IN' });
    }
...
/**
 * 最近 days 天的签到日历与连签天数。
 * created_at 存的是 UTC ISO 串，按字符串切片取日期会在 UTC+8 下把今天算成昨天，
 * 所以统一用服务端本地日历还原日期——与 dailyCheckIn 的「今日」判定同一套口径。
 * 今天尚未签到不计断：连到昨天的那串仍是活的连签。
 */
export async function getDailyCheckInHistory(userId, days = 7) {
  ...
  const rows = await query(`
    SELECT created_at FROM credit_ledger_v2
    WHERE user_id = $1 AND action_type = 'DAILY_CHECKIN' AND created_at >= $2
  `, [userId, start.toISOString()]);
  const checkedDates = new Set(rows.rows.map((row) => localDateKey(new Date(row.created_at))));

  const keys = Array.from({ length: window }, (_, index) => {
    const day = new Date(start);
    day.setDate(start.getDate() + index);
    return localDateKey(day);
  });

  const todayKey = keys[window - 1];
  const todayChecked = checkedDates.has(todayKey);
  const runEnd = todayChecked ? window - 1 : window - 2;
  let streak = 0;
  for (let index = runEnd; index >= 0 && checkedDates.has(keys[index]); index -= 1) streak += 1;
  const runStart = runEnd - streak + 1;

  return {
    days: keys.map((key, index) => ({
      date: key,
      checkedIn: checkedDates.has(key),
      ordinal: streak > 0 && index >= runStart && index <= runEnd ? index - runStart + 1 : null,
    })),
    todayChecked,
    today: todayKey,
    streak,
