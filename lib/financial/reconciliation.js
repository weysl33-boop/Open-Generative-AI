...
import * as reconciliationRepo from '../repositories/reconciliation.js';
...
export async function runDailyReconciliation() {
  ...
  const { totalDebit: rawDebit, totalCredit: rawCredit } = await reconciliationRepo.getDoubleEntryTotals();

  const totalDebit = Number(Number(rawDebit).toFixed(4));
  const totalCredit = Number(Number(rawCredit).toFixed(4));
  ...
  // 2. 用户货币钱包物化余额对账
  const wallets = await reconciliationRepo.listCurrencyWalletBalances();

  for (const w of wallets) {
    const userLedger = await reconciliationRepo.getUserCurrencyLedgerTotals(w.user_id);
  ...
  // 3. 算力积分永久池流水对账
  const creditWallets = await reconciliationRepo.listCreditWalletBalances();

  for (const cw of creditWallets) {
    const ledgerSum = await reconciliationRepo.getUserPerpetualCreditLedgerTotal(cw.user_id);
