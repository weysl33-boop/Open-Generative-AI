...
import { AVATAR_FRAMES, findBenefit, isRedeemable } from '../benefits/catalog.js';
...
export async function redeemBenefit({ userId, benefitId, idempotencyKey }) {
  ...
  // 页面上的"准备中"只是展示层，直接 POST 也必须被挡下——目录里占位不等于开放售卖。
  if (!isRedeemable(benefit)) throw domainError('BENEFIT_UNAVAILABLE', '这个项目还在准备中，上线后会第一时间告诉你');
