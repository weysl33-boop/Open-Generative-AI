...
import { queryOne, withTransaction, nowIso, randomId } from '../db/index.js';
...
export async function exchangeCoinToCredits({ userId, coinAmount, idempotencyKey = null }) {
  ...
  if (!Number.isFinite(creditsGained) || creditsGained <= 0) {
  ...
  return await withTransaction(async (tx) => {
    ...
    // 锁定货币钱包
    const currWallet = await tx.queryOne('SELECT * FROM currency_wallets WHERE user_id = $1 FOR UPDATE', [userId]);
    ...
    await tx.execute(`
      ...
      VALUES ($1, $2, 'PERPETUAL', $3, $4, 'COIN_EXCHANGE', $5, $6, $7, $8)
    `, [`cl2_${randomId()}`, userId, creditsGained, newCreditBal, txId, `使用 ${numCoins} K币兑换 ${creditsGained} 算力点`, `${idempotencyKey}:credits`, now]);
...
export async function buyVipWithCoin({ userId, planId, idempotencyKey = null }) {
  if (!idempotencyKey || String(idempotencyKey).trim().length < 8) {
    throw Object.assign(new Error('购买会员必须提供有效的幂等键'), { code: 'IDEMPOTENCY_REQUIRED' });
  }
  const plan = await findPublicPlanById(planId);
  if (!plan || plan.id === 'free') {
    throw new Error('请选择有效的付费会员套餐');
  }

  // 计算所需 K 币（1 K币 = 1 USD，以美元为基础单位换算，对外不展示具体汇率）
  const usdPrice = plan.monthlyUsd || (plan.monthlyCny / (CURRENCY.FLOAT_CNY_USD_RATE || 7.23));
  const requiredCoins = Math.ceil(Number(usdPrice * CURRENCY.USD_TO_COIN_RATE));
  ...
  return await withTransaction(async (tx) => {
    const existing = await tx.queryOne(
      'SELECT transaction_id FROM currency_journal_entries WHERE idempotency_key = $1 FOR UPDATE',
      [idempotencyKey],
    );
    if (existing) return { success: true, idempotent: true, planId: plan.id, transactionId: existing.transaction_id };
    ...
    if (subCreditsBonus > 0) {
      ...
      const updatedCreditWallet = await tx.queryOne(`
        SELECT daily_free_credits, subscription_credits, perpetual_credits
        FROM credit_wallets
        WHERE user_id = $1
      `, [userId]);
      const subscriptionBalanceAfter = Number(updatedCreditWallet?.subscription_credits || 0);
      const aggregateCredits = Number(updatedCreditWallet?.daily_free_credits || 0)
        + subscriptionBalanceAfter + Number(updatedCreditWallet?.perpetual_credits || 0);
      await tx.execute('UPDATE users SET credits = $1, updated_at = $2 WHERE id = $3', [aggregateCredits, now, userId]);

      await tx.execute(`
        INSERT INTO credit_ledger_v2
        (id, user_id, bucket_type, delta, balance_after, action_type, reference_id, description, created_at)
        VALUES ($1, $2, 'SUBSCRIPTION', $3, $4, 'VIP_GRANT', $5, $6, $7, $8)
      `, [`cl2_${randomId()}`, userId, subCreditsBonus, subscriptionBalanceAfter, txId, `开通 ${plan.name} 附赠 ${subCreditsBonus} 算力点`, `${idempotencyKey}:subscription`, now]);
    }
