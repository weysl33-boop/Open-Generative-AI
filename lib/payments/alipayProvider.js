 ...
 * 校验支付宝网关同步响应签名 (RSA2)
 *
 * 待签名串是原始报文里 "<method>_response": 之后、,"sign":" 之前的那段字节。
 * 不能用 JSON.parse 之后再 stringify 的结果去验：键序与转义一旦被归一化就永远验不过。
 *
 * 为什么必须验：查单响应只要写一句 trade_status=TRADE_SUCCESS，轮询就会自己给用户发货，
 * 这是一条绕过异步通知验签的入账路径（DNS/证书/出口被劫持时尤其成立）。
 */
export function verifyAlipayResponseSign(rawBody, responseKey, alipayPublicKey) {
  if (!alipayPublicKey || typeof rawBody !== 'string' || !responseKey) return false;
  const prefix = `"${responseKey}":`;
  const start = rawBody.indexOf(prefix);
  if (start === -1) return false;
  // sign 恒在响应体之后；从尾部找，避开业务字段里可能出现的同名键。
  const signAt = rawBody.lastIndexOf(',"sign":"');
  if (signAt <= start) return false;
  const signStart = signAt + ',"sign":"'.length;
  const signEnd = rawBody.indexOf('"', signStart);
  const sign = signEnd === -1 ? '' : rawBody.slice(signStart, signEnd);
  if (!sign) return false;
  const verifier = crypto.createVerify('RSA-SHA256');
  verifier.update(rawBody.slice(start + prefix.length, signAt), 'utf8');
  return verifier.verify(formatPublicKey(alipayPublicKey), sign, 'base64');
}
...
export function getAlipayProvider({
  ...
  async function requestAlipayApi(method, bizContent) {
    ...
    const signValid = responseBody
      ? verifyAlipayResponseSign(rawBody, responseKey, publicKey)
      : false;

    if (!responseBody || responseBody.code !== '10000') {
      // 错误响应没有授权能力，照原样抛出即可；但签名对不上必须留痕——
      // 那是「支付宝公钥配错」在现场唯一的证据，配错的公钥会让到账通知永远验不过。
      if (responseBody && !signValid) {
    ...
    if (!signValid) {
      ...
      throw Object.assign(new Error('支付宝同步响应验签失败'), {
        code: 'RESPONSE_SIGNATURE_INVALID',
        alipayCode: responseBody.code,
        alipaySubCode: responseBody.sub_code || null,
        alipayMsg: responseBody.msg || null,
        alipaySubMsg: responseBody.sub_msg || null,
      });
    ...
    async createCheckout({ order, user, plan, successUrl, cancelUrl }) {
      ...
      return call('createCheckout', async () => {
        ...
        try {
          ...
          // 临时桥接，不是兜底假数据：本 AppID 未签约「当面付」，precreate 固定回 40004 /
          // ACQ.ACCESS_FORBIDDEN，此时只能报错的话支付宝渠道一分钱也收不到。改投支付宝官方
          // 电脑网站支付网关，params 里带真实 RSA2 签名，扫码后进的是支付宝自己的收银台，
          // 扣款与异步通知都是真的。签约到位后删掉这一段，只保留上面的 res.qr_code。
          const errorCode = String(error?.code || '');
          if (errorCode !== 'ACQ.ACCESS_FORBIDDEN' && errorCode !== 'ACCESS_FORBIDDEN') throw error;
          const pageBizContent = {
            ...bizContent,
          ...
          const timestamp = new Date(Date.now() + 8 * 3600 * 1000).toISOString().replace('T', ' ').slice(0, 19);
          const publicOrigin = String(process.env.PUBLIC_APP_URL || 'https://www.koyosim.com').replace(/\/+$/, '');
          const pageParams = {
            app_id: appId,
            ...
            timestamp,
            version: '1.0',
            notify_url: notifyUrl || publicOrigin + '/api/billing/webhooks/alipay',
            return_url: successUrl || publicOrigin + '/studio',
            biz_content: JSON.stringify(pageBizContent),
          };
          pageParams.sign = signAlipayParams(pageParams, privateKey);
          const cashierUrl = gateway + '?' + new URLSearchParams(pageParams).toString();
          logPaymentEvent('payment.alipay.precreate.success', {
            orderId: order.id,
            outTradeNo: bizContent.out_trade_no,
            status: 'PENDING',
            code: 'ALIPAY_PAGE_PAY_BRIDGE',
            amountMinor: order.amount_minor,
            currency: order.currency,
          });
          return {
