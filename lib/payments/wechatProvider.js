...
function readKeyOrPath(val, pathVal) {
  ...
  if (pathVal && String(pathVal).trim()) {
    ...
    try {
      ...
      console.warn('[wechatProvider] 读取证书/私钥文件失败:', trimmedPath, e.message);
    }
  }
  return null;
}
...
export function getWechatProvider({
  ...
  privateKey = readKeyOrPath(process.env.WECHAT_PRIVATE_KEY, process.env.WECHAT_PRIVATE_KEY_PATH),
  publicKey = readKeyOrPath(process.env.WECHAT_PUBLIC_KEY, process.env.WECHAT_CERT_PATH),
  notifyUrl = process.env.WECHAT_NOTIFY_URL,
  onCall = null,
} = {}) {
  const missingKeys = [];
  if (!appId) missingKeys.push('WECHAT_APP_ID');
  if (!mchId) missingKeys.push('WECHAT_MCH_ID');
  if (!apiV3Key) missingKeys.push('WECHAT_API_V3_KEY');
  if (!serialNo) missingKeys.push('WECHAT_CERT_SERIAL_NO');
  if (!privateKey) missingKeys.push('WECHAT_PRIVATE_KEY(_PATH)');
  ...
  const provider = {
    ...
    async createCheckout({ order, user, plan, successUrl, cancelUrl }) {
      ...
      return call('createCheckout', async () => {
        ...
        const payload = {
          ...
          notify_url: notifyUrl || `${process.env.PUBLIC_APP_URL || 'https://go.koyosim.com'}/api/billing/webhooks/wechat`,
          amount: {
            total: amountTotal,
            currency: 'CNY',
          },
     ...
     * 申请退款
     */
    async refund({ paymentIntentId, outTradeNo, refundAmount, totalAmount, reason = '协商退款' }) {
      if (!isConfigured) return { success: true, mock: true };
      return call('refund', async () => {
        const urlPath = '/v3/refund/domestic/refunds';
        const timestamp = String(Math.floor(Date.now() / 1000));
        const nonce = randomNonce();
        const outRefundNo = `ref_${Date.now()}_${randomNonce(6)}`;
    ...
    verifyWebhookSignature(rawBody, headers) {
      ...
      if (bodyObj.event_type === 'TRANSACTION.SUCCESS' && bodyObj.resource) {
        return decryptWechatV3Resource(bodyObj.resource, apiV3Key);
      }
      return bodyObj;
    },
  };
