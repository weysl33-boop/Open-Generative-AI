...
export function getLocaleFromPathname(pathname) {
  return matchPathLocale(pathname) || DEFAULT_LOCALE;
}

// The locale a path *asserts*, or null when it asserts none. Kept separate
// from getLocaleFromPathname because that function answers DEFAULT_LOCALE for
// every unprefixed route, and `getLocaleFromPathname(p) || cookie` therefore
// never reaches the cookie — an unprefixed path is silence, not "english".
export function matchPathLocale(pathname) {
...
// The one client-side locale answer, so no component picks its own fallback.
// Order mirrors middleware.js: an explicit server-resolved value (the
// `x-locale` header a page wrapper passes down) outranks the URL, which
// outranks the signed-in user's stored preference, which outranks the cookie.
// Server-side there is no document, so this degrades to path-then-default.
export function resolveClientLocale({ explicit, pathname, userLocale, search } = {}) {
  const params = typeof search === 'string' ? new URLSearchParams(search) : null;
  const candidates = [
    explicit,
    matchPathLocale(pathname),
    params?.get('lang') || params?.get('locale'),
    userLocale,
    readCookieLocale(),
  ];
  for (const candidate of candidates) {
    if (isSupportedLocale(candidate)) return normalizeLocale(candidate);
  }
