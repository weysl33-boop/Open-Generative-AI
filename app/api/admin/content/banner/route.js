...
import { getBannerConfig, saveBannerConfig } from '@/lib/services/content';
...
async function handleGET(request) {
  ...
  return okResponse(banner, guard.requestId);
}
...
async function handlePOST(request) {
  ...
  const idemp = await checkIdempotency({
    scope: 'content_banner_update',
    key: idempotencyKey,
    actorId: guard.user.id,
  });
  if (!idemp.allowed) {
    if (idemp.cachedResponse) return okResponse(idemp.cachedResponse, guard.requestId);
    return errorResponse('CONFLICT', '横幅配置更新正在处理中', 409, guard.requestId);
  }
  ...
  const result = await saveBannerConfig({
    actor: guard.user,
    config: body,
  ...
  await completeIdempotency(idemp.keyHash, result.banner);
